...
function requiredDimensionsForSlot(slot: "large" | "side") {
  return slot === "large"
...
export async function POST(request: Request) {
  ...
  if (slotRaw !== "large" && slotRaw !== "side") {
    return NextResponse.json({ error: "Geçersiz görsel alanı." }, { status: 400 });
  }
  ...
  if (dimensions.width !== required.width || dimensions.height !== required.height) {
    return NextResponse.json(
      {
        error: `Görsel tam olarak ${required.width}x${required.height}px olmalıdır.`,
      },
      { status: 400 },
    );
  }
  ...
  if (!supabase) {
    ...
    return NextResponse.json({
      image_url: `https://example.com/${session.tenant!.id}/hero_cluster_${slot}_${Date.now()}.jpg`,
    });
  }

  const filePath = buildHeroClusterImagePath({
...
export async function DELETE(request: Request) {
  ...
  const { error } = await supabase.storage.from(STOREFRONT_BANNERS_BUCKET).remove([objectPath]);
