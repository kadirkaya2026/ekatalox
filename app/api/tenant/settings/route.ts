...
export async function PATCH(request: Request) {
  ...
  const parsed = storefrontSettingsSchema.safeParse({
    ...
    product_image_background:
      body.product_image_background ?? existingSettings.product_image_background,
    ...
    is_logout_button_visible:
      body.is_logout_button_visible ?? existingSettings.is_logout_button_visible,
    ...
    is_always_open: body.is_always_open ?? existingSettings.is_always_open,
    business_hours: body.business_hours ?? existingSettings.business_hours,
    ...
    is_delivery_fee_active:
      body.is_delivery_fee_active ?? existingSettings.is_delivery_fee_active,
    delivery_fee_amount:
      body.delivery_fee_amount ?? existingSettings.delivery_fee_amount,
    delivery_fee_free_threshold:
      body.delivery_fee_free_threshold ?? existingSettings.delivery_fee_free_threshold,
    ...
    cart_form_config:
      "cart_form_config" in body ? body.cart_form_config : existingSettings.cart_form_config,
  });
  ...
  if (!supabase) {
    ...
    return NextResponse.json({
      ...
      storefrontSettings: {
        ...
        is_logout_button_visible: parsed.data.is_logout_button_visible,
        ...
        is_always_open: parsed.data.is_always_open,
        ...
        is_delivery_fee_active: parsed.data.is_delivery_fee_active,
        delivery_fee_amount: parsed.data.delivery_fee_amount,
        delivery_fee_free_threshold: parsed.data.delivery_fee_free_threshold,
    ...
    cart_form_config: parsed.data.cart_form_config,
      },
    });
  }
  ...
  const previousManagedBannerPaths = new Set(
    [...(existingSettings.banner_items ?? []), ...(existingSettings.hero_cluster_items ?? [])]
  ...
  const nextManagedBannerPaths = new Set(
    [...parsed.data.banner_items, ...parsed.data.hero_cluster_items]
      .map((item) => getBannerObjectPath(item.image_url))
      .filter((path): path is string => typeof path === "string")
      .filter((path) => path.startsWith(`${session.tenant!.id}/`)),
  );
