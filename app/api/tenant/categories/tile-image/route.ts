...
async function getCategory(categoryId: string, tenantId: string) {
...
export async function POST(request: Request) {
  ...
  if (!categoryId) {
    return NextResponse.json({ error: "Kategori kimliği zorunludur." }, { status: 400 });
  }
  ...
  if (!supabase) {
    ...
    return NextResponse.json({
      category: {
        id: categoryId,
        tile_image_url: `https://example.com/${session.tenant!.id}/category_${categoryId}/tile_${Date.now()}.jpg`,
      },
    });
  }

  const existingCategory = await getCategory(categoryId, session.tenant!.id);
  ...
  if (image.size > maxBannerFileSizeBytes) {
    return NextResponse.json(
      { error: "Görsel en fazla 2MB olabilir." },
      { status: 400 },
    );
  }
  ...
  if (!dimensions) {
    return NextResponse.json(
      { error: "Görselin çözünürlüğü okunamadı." },
      { status: 400 },
    );
  }
  ...
  if (
    ...
    return NextResponse.json(
      {
        error: `Görsel tam olarak ${requiredCategoryTileWidth}x${requiredCategoryTileHeight}px olmalıdır.`,
      },
      { status: 400 },
    );
  }

  const filePath = buildCategoryTilePath({
  ...
  const { data, error } = await supabase
    .from("categories")
    .update({ tile_image_url: publicUrlData.publicUrl })
    .eq("id", categoryId)
  ...
  if (error) {
    await supabase.storage.from(STOREFRONT_BANNERS_BUCKET).remove([filePath]);
    return NextResponse.json({ error: error.message }, { status: 400 });
  }

  const previousImageUrl = existingCategory.tile_image_url as string | null;
...
export async function DELETE(request: Request) {
  ...
  const session = await getSessionContext();
  const { category_id: categoryId } = (await request.json()) as {
    category_id?: string;
  };
  ...
  const previousImageUrl = existingCategory.tile_image_url as string | null;
  const objectPath = getBannerObjectPath(previousImageUrl);
