...
export async function POST(request: Request) {
  ...
  const categoryExists = await ensureCategoryBelongsToTenant(
    categoryId,
    session.tenant!.id,
  );
  ...
  if (!supabase) {
    ...
    return NextResponse.json({
      image_url: `https://example.com/${session.tenant!.id}/category_${categoryId}/banner_${Date.now()}.jpg`,
    });
  }

  const filePath = buildCategoryBannerPath({
...
export async function DELETE(request: Request) {
  ...
  const session = await getSessionContext();
  const { image_url, category_id: categoryId } = (await request.json()) as {
    image_url?: string;
    category_id?: string;
  };
