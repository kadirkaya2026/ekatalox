...
const Page = () => {
  return (
    <main className="relative min-h-screen bg-[#0B0F19] text-white overflow-hidden">
      ...
      <PageHero
        tag="Müşterilerimiz"
        title={<>850+ toptancı zaten <span className="bg-clip-text text-transparent bg-gradient-to-r from-[#00D2FF] to-[#7928CA]">eKatalox’ta.</span></>}
          ...
          <div className="space-y-5">
            {cases.map((c, i) => (
              <motion.div key={c.company} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: false, amount: 0.2 }} transition={{ duration: 0.7, delay: i * 0.08 }} className="relative rounded-3xl border border-white/10 bg-gradient-to-b from-white/[0.04] to-transparent p-7 md:p-10 overflow-hidden">
                <div className="absolute -top-20 -right-20 w-60 h-60 rounded-full bg-[#00D2FF]/10 blur-3xl pointer-events-none" />
                <div className="grid md:grid-cols-[1.5fr_1fr] gap-8 relative">
                  <div>
                    <div className="flex items-center gap-3 mb-4">
                      <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-[#00D2FF]/30 to-[#7928CA]/30 border border-white/10 flex items-center justify-center text-sm font-bold text-white">{c.company[0]}</div>
                      <div>
                        <div className="text-white font-semibold">{c.company}</div>
                        <div className="text-xs text-slate-500">{c.sector}</div>
                      </div>
                    </div>
                    <Quote className="w-6 h-6 text-[#00D2FF] mb-2" />
                  ...
                  <div className="grid grid-cols-3 md:grid-cols-1 gap-3">
                    {c.metrics.map(([v, l]) => (
                      <div key={l} className="p-4 rounded-xl border border-white/10 bg-white/[0.02] text-center">
                        <div className="text-xl md:text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-[#00D2FF] to-[#7928CA]">{v}</div>
            ...
            {testimonials.map((t, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: false }} transition={{ delay: i * 0.05 }} whileHover={{ y: -4 }} className="p-6 rounded-2xl border border-white/10 bg-white/[0.02] hover:border-white/20 transition-colors">
                <div className="flex gap-0.5 mb-3">
                  {[1,2,3,4,5].map(s => <Star key={s} className="w-3.5 h-3.5 fill-[#00D2FF] text-[#00D2FF]" />)}
                </div>
      ...
      <section className="px-6 pb-32">
        <div className="max-w-3xl mx-auto text-center">
          <h3 className="text-3xl md:text-4xl font-bold tracking-tight" style={{ letterSpacing: '-0.04em' }}>Sıradaki başarı hikayesi <span className="bg-clip-text text-transparent bg-gradient-to-r from-[#00D2FF] to-[#7928CA]">sizinki olsun.</span></h3>
